...
public class CORStripes extends Configured implements Tool {
    ...
    private static class CORMapper1 extends
        ...
        public void map(LongWritable key, Text value, Context context)
            ...
            // Emit single word counts
            for (String word : word_set.keySet()) {
                WORD.set(word);
                COUNT.set(word_set.get(word));
    ...
    public static class CORStripesCombiner2 extends Reducer<Text, MapWritable, Text, MapWritable> {
        ...
        protected void reduce(Text key, Iterable<MapWritable> values, Context context)
            ...
            // Sum the occurrences of word pairs in the stripe
            for (MapWritable val : values) {
                for (Map.Entry<Writable, Writable> entry : val.entrySet()) {
                    ...
                    stripe.put(word,
                            new IntWritable(
                                    stripe.containsKey(word) ? ((IntWritable) stripe.get(word)).get() + count.get()
                                            : count.get()));
                }
            }
    ...
    public static class CORStripesReducer2 extends Reducer<Text, MapWritable, PairOfStrings, DoubleWritable> {
        ...
        protected void reduce(Text key, Iterable<MapWritable> values, Context context)
            ...
            // Merge all stripes for this key
            for (MapWritable val : values) {
                for (Map.Entry<Writable, Writable> entry : val.entrySet()) {
                    Text word = (Text) entry.getKey();
                    IntWritable count = (IntWritable) entry.getValue();
            ...
            for (Map.Entry<Writable, Writable> entry : stripe.entrySet()) {
                ...
                if (word_total_map.containsKey(wordA) && word_total_map.containsKey(wordB.toString())) {
                    int FreqA_FreqB = word_total_map.get(wordA) * word_total_map.get(wordB.toString());
                    double COR = (double) cooccurrence / FreqA_FreqB;
                    context.write(new PairOfStrings(wordA, wordB.toString()), new DoubleWritable(COR));
                }
            }
        }
    }
